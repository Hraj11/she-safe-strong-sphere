...
const FeatureCard = ({ icon: Icon, title, description, color, link }: FeatureCardProps) => {
  ...
  return (
    <Link to={link} className="block transition-transform hover:scale-[1.02]">
      <Card
        className={`p-6 bg-gradient-to-br ${colorClasses[color]} border-0 card-glow group cursor-pointer`}
      >
        <div
          className={`w-14 h-14 rounded-2xl ${iconBgClasses[color]} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
        >
